...
## Автор решения
Медвдева Екатерина Александровна 21.03.2002

## Описание реализации
Программа написана на C++. Суть реализации заключается в том, что создается структура для хранения id, ФИО и кол-ва списанных часов для каждого поля из файла и вектор со структурами для хранения данных о каждом сотруднике без повторов. Затем происходит считывание файла "report.txt", откуда информация помещается в структуру. Далее по ID проверяется был ли данный сотрудник записан в вектор. Далее отбираются сотрудники, подходящие под условие для вывода в файл-результат, записываются в отсортированные вектора данных по принципу ключ-значение и в сам файл.

## Инструкция по сборке и запуску решения
Код решения представлен в файле ConsoleApp.cpp, запуск решения осуществляется с помощью файла ConsoleApp.exe. Для корректной работы файл report.txt должен присутствовать в одной директории с ConsoleApp.exe.
