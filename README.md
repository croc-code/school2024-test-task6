...
Для сборки программы нужен компилятор C++, подойдет g++. Создайте текстовый файл с именем report.txt в том же каталоге, где находится исполняемый файл программы, файл должен содержать данные аналогичные из условия. Откройте терминал (командную строку) и перейдите в каталог, где находится ваш файл solution.cpp. Выполните команду для компиляции: g++ -o time_balance time_balance.cpp. После завершения работы программы в каталоге должен появиться файл result.txt. Откройте его для проверки результатов.
