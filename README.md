...
## Автор решения
**Гузенко Сергей Викторович**

## Описание реализации
Класс **ImbalanceSensor** предназначен для обработки файла отчета, содержащего данные о работниках, и расчета их дисбаланса на основе стандартного значения. Вот краткое описание его реализации:

`__init__:`
Инициализирует объект ImbalanceSensor с путем к файлу отчета. Если путь не указан, он по умолчанию устанавливается как **"report.txt"**.

`get_result:`
Обрабатывает файл отчета, вычисляет дисбаланс и записывает результаты в файл. Принимает опциональный параметр **result_path** для указания пути к файлу результата.

`get_standard:`
Считывает норму работы (первая строка файла отчета) и возвращает его в виде числа с плавающей точкой. Если файл не найден или значение не является допустимым, возвращает None.

`get_report:`
Считывает файл отчета, пропускает первую строку, и выдает каждую строку в виде итератора. Если строка не содержит 6 элементов (ожидаемый формат), выдает None.

`abbreviate_name:`
Принимает полное имя в качестве входных данных и возвращает его сокращенную версию.

## Инструкция по сборке и запуску решения
Достаточно **просто разместить файл с именем report.txt** в директорию, в которой находится `ImbalanceSensor` (*корневая*) и запустить `ImbalanceSensor.py`