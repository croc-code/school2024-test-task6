...
## Автор решения
Шаров Матвей Андреевич
## Описание реализации
Исходя из условия задачи можно предположить, что диапзон дат ограничиавется одной неделью, поэтому отдельной обработки иного не было реализовано.

В конструкторе ```Solution``` выполняется основной жизненный цикл программы:

1. Парсинг аргументов с помощью ```apache.commons``` - метод ```parseArgs```.
2. В методе ```readData``` считываются все пользователи из входного файла в ассоциативный массив.
3. В методе ```writeData``` происходит запись работников, обработанных в методе ```defineEmployees```, в выходной файл.
## Инструкция по сборке и запуску решения
Все комнады выполняются в папке нахождения ```task6-1.0-SNAPSHOT.jar```

1. Архив можно получить в папке ```target``` после выполнения команды ```mvn package``` или воспользовыться готовым в папке ```ready```.
2. Распаковка jar-архива

   ```jar -xf task6-1.0-SNAPSHOT.jar commons-cli-1.7.0.jar```
3. Запуск

   ```java -jar task6-1.0-SNAPSHOT.jar```

   Необязательными параметрами являются ```--input=[путь к файлу] --output=[путь к файлу]```.
   
   Если параметры не указаны, то ```--input=.\report.txt```, ```--output=.\result.txt```.
