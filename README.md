...
## Автор решения
Зуборев Николай Александрович tg:@nikolzu

## Описание реализации

### Метод `analyzeTimeBalance`:

- Принимает имя файла, со списками списаний времени сотрудников.
- Считывает недельную норму из файла и сохраняет ее в атрибут `weeklyNorm`.
- Читает списки списания времени для каждого сотрудника и сохраняет их в `timeSpent` и `employeeNames`.
- Вычисляет дисбаланс списания времени с помощью метода `calculateImbalance`.
- Записывает результаты анализа в файл с помощью метода `writeResults`.

### Вспомогательные методы:

- **`readWeeklyNorm`**: возвращает недельную норму из файла.
- **`readTimeEntries`**: считывает списания времени для сотрудника из файла.
- **`processTimeEntry`**: обрабатывает запись о списании времени для одного сотрудника и сохраняет данные в `timeSpent` и `employeeNames`.
- **`formatEmployeeName`**: форматирует имя сотрудника в нужный формат.
- **`calculateImbalance`**: вычисляет разницу списания времени сотрудниками и возвращает его.
- **`writeResults`**: записывает результаты анализа в файл, сначала отрицательную разницу, затем положительную, сортируя ФИО по алфавиту.

### Сортировка:

- Внутри каждой категории сортируются по алфавиту ФИО.

## Инструкция по сборке и запуску решения
```
в файле report.txt должны быть записи в формате TSR-3
После инструкций приведенных ниже, в папке проекта появится файл result.txt с результатами анализа
```
Использовал Maven для сборки

- mvn compile - для компиляции 
- mvn test - для запуска тестов
- mvn exec:java -Dexec.mainClass="Main" -Dexec.args="report.txt" (или имя другого файла для тестирования)

```sh
mvn compile
mvn test
mvn exec:java -Dexec.mainClass="Main" -Dexec.args="report.txt" 
