...
## Описание реализации
В начале программы подключаются все необходимые библиотеки, открываются входной файл report.txt для чтения данных и выходной файл result.txt для записи результатов, используются два словаря для хранения данных о сотрудниках, далее читаются и обрабатываются все строки из файла, кроме первой. Для каждой строки данные разделяются на части, извлекаются необходимые поля и обновляются соответствующие словари. Для каждого сотрудника вычисляется дизбаланс по времени. Если дизбаланс превышает 10% от недельной нормы, он добавляется в результирующий вектор. Результаты сортируются сначала по знаку дизбаланса, а затем по фамилии сотрудников. Результаты записываются в выходной файл в требуемом формате. После завершения работы с файлами, они закрываются.

## Инструкция по сборке и запуску решения
Для сборки программы нужен компилятор C++, подойдет g++. Создайте текстовый файл с именем report.txt в том же каталоге, где находится исполняемый файл программы, файл должен содержать данные аналогичные из условия. Откройте терминал (командную строку) и перейдите в каталог, где находится ваш файл solution.cpp. Откройте Visual Studio и создайте новый проект C++. Добавьте solution.cpp в проект и нажмите "Build" для сборки. После завершения работы программы в каталоге должен появиться файл result.txt. Откройте его для проверки результатов.
