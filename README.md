...
### Запуск программы через .jar файл

1. **Сборка jar файла**:
   - В IntelliJ IDEA перейдите в `File > Project Structure`.
   - Выберите вкладку "Artifacts".
   - Нажмите на значок `+` и выберите "Jar > From modules with dependencies".
   - Выберите главный класс (`src.Solution.Main`) и нажмите "OK".
   - В разделе "Output directory" выберите папку, куда будет сохранен jar файл.
   - Нажмите "Apply" и затем "OK".
   - Перейдите в `Build > Build Artifacts` и выберите "Build".
...
Так же можете собрать .jar самостоятельно.
Снизу представлена инструкция
