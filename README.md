...
## Автор решения - Вильданов Искандер Рустамович

## Описание реализации
Программа ImbalanceAnalyzer представляет собой утилиту, которая считывает данные из файла report.txt в формате TSR-3, проводит анализ дизбаланса и в выходном файле result.txt перечисляет сотрудников, имеющих какой либо дизбаланс.   

## Инструкция по сборке и запуску решения
Для запуска программы необходимо воспользоваться командой `dotnet run` из директории проекта.
- Необходимым условием для правильности работы программы является наличие файла report.txt в формате TSR-3 внутри директории `/ImbalanceAnalyzer`
- По завершении программы будет сформирован файл result.txt, в котором будут перечислены сотрудники с дизбалансом. Файл будет находится в той же директории, что и report.txt