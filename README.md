...
## Описание реализации
Сначала читается входной файл, из первой строки извлекается недельная норма списания, а последующие строки парсятся для получения данных о времени списаний сотрудников, которые суммируются и сохраняются по их уникальным идентификаторам (ID) в коллекцию timeEntries. Одновременно создается коллекция employeeNames, хранящая соответствие между ID и ФИО сотрудников. Затем определяется баланс времени для каждого сотрудника, и те, у кого дизбаланс превышает 10% от нормы, добавляются в списки underBalanced или overBalanced в зависимости от недобора или перебора времени. Эти списки сортируются по фамилии сотрудников и объединяются в итоговый список, который записывается в выходной файл. В случае возникновения ошибок программа обрабатывает исключения и выводит сообщения об ошибках.

## Инструкция по сборке и запуску решения
1. Создайте директорию для проекта, поместите туда файл TimeBalanceAnalyzer.java с исходным кодом и файл report.txt с входными данными.
2. Откройте командную строку, перейдите в директорию проекта, выполните команду javac TimeBalanceAnalyzer.java для компиляции и затем java TimeBalanceAnalyzer для запуска программы.
3. Результаты будут записаны в файл result.txt в той же директории.
