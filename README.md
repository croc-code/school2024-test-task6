...
Класс Employee используется для хранения состояния сотрудника после чтения файла, сортировки сотрудников и корректного вывода в файл. Информация о UUID и датах списания не использовалась, так как не нужна для вывода в файл.
...
|hoursThisWeek| `double` |Суммарное количество списанных часов|
|hoursImbalance| `double` |Дисбаланс списанных часов по сравнению с недельной нормой|
...
Класс FileHandler используется для обработки файлов, включая чтение данных сотрудников из файла и запись отсортированных данных обратно в файл.

|Поле|Тип|Описание|
|:---|:--|:-------|
|filePathToRead| `String` |Путь к файлу для чтения исходных данных|
|filePathToWrite| `String` |Путь к файлу для записи обработанных данных|

**Методы класса:** (помимо сеттеров)

* **readFile()**: Метод устанавливает недельную норму часов и читает данные сотрудников из файла, указанного в filePathToRead. Он парсит каждую строку для извлечения данных о сотрудниках и сохраняет их в мапу, где ключом является UUID, а значением - объект класса Employee. При наличии сотрудника в мапе увеличиваются списанные часы. В конце метод возвращает список уникальных сотрудников.
...
Класс DataAnalyzer используется для анализа рабочего времени сотрудников, определяя дисбаланс рабочих часов по сравнению с недельной нормой и сортируя их согласно техническому заданию.
...
* **pulbic static void main(String[] args)** Главный метод приложения, который выполняет следующие шаги:
    ...
    * Вызывает метод для анализа рабочих часов сотрудников на предмет дисбаланса более 10% класса DataAnalyzer.
...
Для запуска вы можете скачать [school-test.jar](), который располагается по адресу:
```bash

```
Затем вам необходимо, перейти в папку с jar архивом, поместить в эту папку входной файл с данными и выходной файл для вывода результата и прописать следующую команду:
```bash
